...
class TMDbClient {
   ...
   * Search for TV shows
   */
  async searchShows(query: string, page = 1): Promise<TMDbSearchResult<TMDbShow>> {
  ...
  async getAllEpisodes(showId: number): Promise<TMDbSeasonDetails[]> {
    ...
    for (const season of showDetails.seasons) {
      ...
      try {
        const seasonDetails = await this.getSeasonDetails(showId, season.season_number);
        seasons.push(seasonDetails);
      } catch (error) {
        console.error(`Failed to fetch season ${season.season_number} for show ${showId}:`, error);
      }
    }
  ...
  static getBackdropUrl(
    ...
    return TMDbClient.getImageUrl(
      path,
      size as "w92" | "w154" | "w185" | "w342" | "w500" | "w780" | "original",
    );
  }
