...
try {
  ...
  console.log(
    "[TMDb Server] Episode database loaded:",
    episodeDatabase?.episodes?.length,
    "episodes",
  );
} catch {
  ...
  async (): Promise<ShowWithEpisodes[]> => {
    ...
    for (const show of CRIME_SHOWS) {
      try {
        ...
        const seasonDetails = await tmdb.getSeasonDetails(show.tmdbId, latestSeasonNumber);
        ...
        const episodes: EpisodeData[] = sortedEpisodes.map((ep) => {
          ...
          if (episodeDatabase?.episodes) {
            ...
            relatedShows = [
              ...new Set(
                related.filter((r) => r.showTmdbId !== show.tmdbId).map((r) => r.showName),
              ),
            ];
          }
  ...
  .handler(async ({ data }): Promise<{ episodes: EpisodeData[]; hasMore: boolean }> => {
    ...
    const episodes: EpisodeData[] = slicedEpisodes.map(({ episode, showName }) => {
      ...
      if (episodeDatabase?.episodes) {
        ...
        // Get unique show names from related episodes (excluding current show)
        relatedShows = [
          ...new Set(related.filter((r) => r.showTmdbId !== data.showId).map((r) => r.showName)),
        ];
      }
...
export const getEpisode = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const episode = seasonDetails.episodes.find((ep) => ep.episode_number === data.episodeNumber);
...
export const getEpisodeById = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    for (const seasonNum of seasonNumbers) {
      try {
        const seasonDetails = await tmdb.getSeasonDetails(data.showId, seasonNum);
        const episode = seasonDetails.episodes.find((ep) => ep.id === data.episodeId);
