...
function extractNames(text: string): string[] {
  ...
  const contextPattern =
    /(?:victim|accused|suspect|defendant|nurse|doctor|Dr\.|specialist)\s+([A-Z][a-z]+\s+[A-Z][a-z]+)\b/gi;
...
function extractLocations(text: string): string[] {
  ...
  const cityStatePattern =
    /\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?),\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?|[A-Z]{2})\b/g;
...
export function calculateMatchScore(
  ...
  const yearMatches = sourceTerms.years.filter((year) => targetTerms.years.includes(year));
