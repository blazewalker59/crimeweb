...
Drizzle cannot express CHECK constraints, so the "exactly one target" rule has to be added by hand — and **it must be inline in the `CREATE TABLE`**. SQLite has no `ALTER TABLE … ADD CONSTRAINT`; verified against D1, which rejects it with `near "CONSTRAINT": syntax error`. Edit the generated migration so the table is created as:

```sql
CREATE TABLE `coverage` (
  -- ... columns as generated by drizzle-kit ...
  CHECK ((`case_id` IS NOT NULL) <> (`series_id` IS NOT NULL))
);
```

Verified on D1: this accepts a row with exactly one target and rejects one with both, raising `SQLITE_CONSTRAINT_CHECK`. Adding the constraint later would require SQLite's twelve-step table rebuild, so getting it into the initial migration matters.
